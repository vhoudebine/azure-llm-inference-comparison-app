...
def ai_inference_stream(ctx, 
    ...
    try:
        ...
        results[index]= {"response": assistant_reply,
                ...
                "output_tokens":utils.num_tokens_from_string(assistant_reply, "gpt-4o")}
...
prompt_container = headcol2.empty()
...
if prompt := st.chat_input("Enter a prompt"):
    ...
    if missing:
        ...
        threads = []
        for i in range(2):
            thread = threading.Thread(target=ai_inference_stream, args=(ctx,prompt,i))
            thread.start()
    ...
    for i in range(2):
        ...
        with collist[1]:
            with st.popover(f"Settings"):
                ...
                st.text_input("Endpoint URL", key=f"url_{i+1}")
                st.text_input("API Key", type="password", key=f"key_{i+1}")
                st.text_input("Deployment (AOAI Only)", key=f"deployment_{i+1}", help="Deployment name for AOAI models")
